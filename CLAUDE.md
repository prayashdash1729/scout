...
modules/scorer.py        → Module 2: score jobs against CV using Claude API
...
- [x] modules/scorer.py — Claude-powered CV vs job scoring (returns score/10 + metadata)
...
- **Tavily** for web search (clean API, built for agents)
- **Claude claude-sonnet-4-5** for all LLM calls (scoring, drafting, reasoning)
